...
class MainFrame(Frame):
	...
	self.by = StringVar()
        ...
        texto = Text(self, height=20, width=50).grid(\
    ...
    def makeToolBar(self):
        ...
        Button(toolbar, text="Sair", \
                 command=self.sair).grid(row=0,column=6)
        Button(toolbar, text="Limpa tela", \
                 command=self.limpaCampos).grid(row=0,column=5)
    ...
    def procurar(self):
        ...
        if not self.db.has_key(nome):
            Dialog(self, title="Erro!", text="Nome não registrado", \
                         bitmap='error', default=0, strings=('OK',))
            return
    ...
    def remover(self):
        ...
        resposta = Dialog(self, title="Confirmação de segurança", \
